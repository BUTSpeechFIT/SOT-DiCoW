...
from models.dicow.encoder import DiCoWEncoder
from models.sot_dicow import SOTDiCoWForConditionalGeneration, WhisperTokenizerForSOT
from models.explicit_sot_dicow import WhisperTokenizerForExplicitSOT, ExplicitSOTDiCoWForConditionalGeneration
...
class WhisperContainer:
    def __init__(self, model_type='whisper-tiny', proc_diar_mask=True, pretrained_encoder=None, ctc_weight=0.0,
                 shift_pos_embeds=False, training_args=None, predict_timestamps=False, global_lang_id="en",
                 vad_seek_callback=None, params_to_keep_frozen_keywords=None, mt_sot=False, mt_num_speakers=1,
                 mt_sot_explicit_speaker_tokens=False, **kwargs):
        self.model_type = model_type
        self.proc_diar_mask = proc_diar_mask  # TODO: proc_diar_mask is never used
        model_class = DiCoWForConditionalGeneration
        if mt_sot and mt_sot_explicit_speaker_tokens:
            model_class = ExplicitSOTDiCoWForConditionalGeneration
        elif mt_sot:
            model_class = SOTDiCoWForConditionalGeneration
        self.model = (model_class
                      .from_pretrained(model_type,
                                       ...
                                       mt_num_speakers=mt_num_speakers,
                                       mt_sot_explicit_speaker_tokens=mt_sot_explicit_speaker_tokens,
                                       **kwargs
                                       )
        ...
        if mt_sot and mt_sot_explicit_speaker_tokens:
            self.model.resize_token_embeddings(self.model.vocab_size)

        self.feature_extractor = WhisperFeatureExtractor.from_pretrained(self.model_type)
        if mt_sot and mt_sot_explicit_speaker_tokens:
            self.tokenizer = WhisperTokenizerForExplicitSOT.from_pretrained(self.model_type, predict_timestamps=predict_timestamps, mt_num_speakers=mt_num_speakers)
        elif mt_sot:
            self.tokenizer = WhisperTokenizerForSOT.from_pretrained(self.model_type, predict_timestamps=predict_timestamps, mt_num_speakers=mt_num_speakers)
        else:
        ...
        if ".en" not in model_type:
            ...
            # This ensures labels
            self.tokenizer.set_prefix_tokens(predict_timestamps=predict_timestamps, task="transcribe", language=global_lang_id)
        else:
        ...
        if pretrained_encoder is not None:
            encoder = DiCoWEncoder.from_pretrained(pretrained_encoder,
                                                   ctc_weight=ctc_weight,
        ...
        if params_to_keep_frozen_keywords:
...
class WhisperQKContainer(WhisperContainer):
    def __init__(self, model_type='whisper-tiny', bias_decoder=True, shift_pos_embeds=False, **kwargs):
        super().__init__(model_type, use_fddts=False,
