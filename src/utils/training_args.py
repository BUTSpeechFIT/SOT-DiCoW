...
from models.sot_dicow.config import SOTAggregationType, SOTStyle
...
class ModelArguments:
    ...
    fddt_init: Optional[str] = field(default="non-disturbing", metadata={
        "help": "FDDT init. Possible methods: random, non-disturbing, dispargement."})

    non_target_fddt_value: Optional[float] = field(default=1.0, metadata={
        "help": "Non target FDDT value for initialization"})
    use_initial_fddt: Optional[bool] = field(default=True, metadata={"help": "Whether to use initial FDDT before adding positional embeddings."})
    ...
    mt_sot: Optional[bool] = field(default=False, metadata={"help": "Multi-talker ASR with serialized output tokens."})
    mt_num_speakers: Optional[int] = field(default=4, metadata={"help": "Number of speakers (channels) in MT-ASR"})
    mt_interactions: Optional[bool] = field(default=True, metadata={
        "help": "Apply Co-Attention across speaker-channels"})
    mt_sot_aggregate_speakers: bool = field(default=False, metadata={
        "help": "Aggregate speaker channels from encoder into single channel"
    })
    mt_sot_aggregation_type: Optional[str] = field(default=None, metadata={
        "help": "Use serializaed output training (SOT) by aggregating over speaker-channels.",
        "choices": [t.value for t in SOTAggregationType]})
    mt_sot_transform_speakers: bool = field(default=False, metadata={
        "help": "Transform speaker channels before aggegating them into single channel."})
    mt_sot_use_sad: bool = field(default=False, metadata={
        "help": "Use speaker activity detection in decoder"})
    mt_sot_spk_mask_inv_temp: Optional[float] = field(default=None, metadata={
        "help": "Softmax inverse temperature (scaler) for speaker masking"})
    mt_sot_encoder: bool = field(default=False, metadata={
        "help": "Use speaker-specific FDDTs in encoder"})
    mt_sot_speaker_loss_weight: float = field(default=0.0, metadata={
        "help": "Speaker loss penalty"})
    mt_sot_speaker_attn_weight: float = field(default=0.0, metadata={
        "help": "Attention weight for speaker-specific encoder's embeddings."})
    mt_sot_explicit_speaker_tokens: bool = field(default=False, metadata={
        "help": "Use explicit speaker tokens in SOT"})
    mt_sot_style: str = field(default="utterance", metadata={
        "help": "SOT style for multi-talker ASR dataset.",
        "choices": [s.value for s in SOTStyle]})
    ...
    def __post_init__(self):
        ...
        # MT-ASR is the main switch
        self.mt_interactions = self.mt_interactions and self.mt_asr
        self.mt_sot_aggregation_type = self.mt_sot_aggregation_type if self.mt_asr else None
        if self.mt_sot_aggregation_type is not None and self.mt_sot_aggregation_type.lower() == "none":
            self.mt_sot_aggregation_type = None
        self.mt_num_speakers = self.mt_num_speakers if self.mt_asr else 1
...
class DataArguments:
    ...
    mt_sot_augment_speaker_order: bool = field(default=False, metadata={
        "help": "Augment train data by reordering speakers"})
    mt_sot_style: str = field(default="utterance", metadata={
        "help": "SOT style for multi-talker ASR dataset.",
        "choices": [s.value for s in SOTStyle]})
    mt_sot_explicit_speaker_tokens: bool = field(default=False, metadata={
...
class DecodingArguments:
    ...
    length_penalty: Optional[float] = field(default=None, metadata={"help": "Length penalty for eval"})
    ...
    soft_vad_temp: Optional[float] = field(default=None, metadata={
        "help": "Apply softmax with the temperature (t > 1 => increases entropy)"})
    num_beams: Optional[int] = field(default=5, metadata={
        "help": "Number of beams for decoding eval dataset (different from dev's training_args.generation_num_beams)"
    })
...
class CustomTrainingArguments(GeneralTrainingArguments):
    ...
    length_penalty: Optional[float] = field(
        default=1.0, metadata={"help": "Length penalty for dev."}
    )
    evaluation_strategy: Any = field(
        default=None,
        metadata={"help": "Deprecated. Use `eval_strategy` instead"},
    )
    mt_sot_decay_params: Optional[List] = field(
        default=None,
        metadata={"help": "List of params to manually decay weights during training, e.g. SOT ."}
    )
    mt_sot_decay_steps: Optional[int] = field(
        default=0,
        metadata={"help": "Number of weight decay steps"}
    )
    log_params: Optional[list] = field(default=None, metadata={"help": "Log these params to wandb and logger"})
...
class Cfg:
    ...
    def save(self, output_file=None):
        if output_file is None:
            output_file = Path(self.training.output_dir) / "experiment_conf.json"
        else:
...
def process_config(cfg: Cfg):
    ngpus = device_count()
    if ngpus > 0:
        accumulution_steps = cfg.training.overall_batch_size // (
                ngpus * cfg.training.per_device_train_batch_size)
    else:
        # GPU is not available
        accumulution_steps = cfg.training.overall_batch_size // cfg.training.per_device_train_batch_size
    cfg.training.gradient_accumulation_steps = accumulution_steps if accumulution_steps > 0 else 1
