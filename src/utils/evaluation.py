...
def parse_string_to_objects(s):
    spkr_time_pattern = re.compile(r'<\|spk([\d]+)\|><\|([\d.]+)\|>')
    if spkr_time_pattern.match(s):
        return parse_sot_transcript(s)

    # Regular expression to match the time tokens
    time_pattern = re.compile(r'<\|([\d.]+)\|>')
    spkr_pattern = re.compile(r'<\|spk([\d]+)\|>')
    ...
    for i in range(0, len(times) - 1):
        ...
        spkr_matched = spkr_pattern.match(text)  # check for spkr tag
        if spkr_matched:
            text = text.replace(spkr_matched.group(), "")
        if not text:  # Only add if there is some text
            continue
        obj = {
            'start': start_time,
            'end': end_time,
        ...
        if spkr_matched:
            obj['speaker'] = f"spk_{spkr_matched.group(1)}"
        objects.append(obj)

    return objects


def parse_sot_transcript(transcript: str):
    # transcript is of the form:
    # <|spk0|><|0.00|>text1<|1.00|>text2<|2.00|>text3<|3.00|>text4<|4.00|>text5<|5.00|>text6<|6.00|>text7<|7.00|>text8<|8.00|>text9<|9.00|>text10<|10.00|>text11<|11.00|>text12<|12.00|>text13<|13.00|>text14<|14.00|>text15<|15.00|>text16<|16.00|>text17<|17.00|>text18<|18.00|>text19<|19.00|>text20<|20.00|>text21<|21.00|>text22<|22.00|>text23<|23.00|>text24<|24.00|>text25<|25.00|>text26<|26.00|>text27<|27.00|>text28<|28.00|>text29<|29.00|>text30<|30.00|>
    # we want to parse it into a list of objects with start, end, and text
    # and speaker id

    # first, we need to find all the time tokens
    # Regular expression to match the time tokens
    time_pattern = re.compile(r'<\|([\d.]+)\|>')
    spkr_pattern = re.compile(r'<\|(spk[\d]+)\|>')

    # Split the text using the speaker tokens to get the text segments
    text_segments = spkr_pattern.split(transcript)
    text_segments = list(filter(None, text_segments))  # Ignore the first empty element

    # Create the list of objects with start, end, and text
    objects = []
    for i in range(0, len(text_segments), 2):
        if i + 1 >= len(text_segments):
            break  # Need both speaker and text

        spk = text_segments[i]
        text_with_times = text_segments[i + 1]

        arr = time_pattern.split(text_with_times)
        arr = list(filter(None, arr))

        if len(arr) == 3:
            start_time, text, end_time = arr
            obj = {
...
def process_session(session_preds, tokenizer, spk_id, cut: DataCut, max_repetitions=3, save_dir=None):
    ...
    if save_dir is not None:
        (Path(save_dir) / 'wer' / cut.recording_id).mkdir(parents=True, exist_ok=True)
        with open(Path(save_dir) / 'wer' / cut.recording_id / "sot_tra.txt", "a") as f:
            f.write(transcript)
            f.write("\n")
    ...
    for segment in segments:
        yield {
            ...
            'speaker_id': segment.get("speaker", spk_id),
...
def compute_longform_metrics_for_sot(pred, trainer, output_dir, text_norm, metrics_list=None, dataset=None):
    # if not main process, return
    metrics = {}
    if trainer.accelerator.is_main_process:
        if dataset is not None:
        ...
        for index, session_preds in enumerate(pred.predictions):
            label_ids = pred.label_ids[index]
            if (session_preds == -100).all():
                continue
            label_ids[label_ids == -100] = trainer.tokenizer.pad_token_id
            cut_id, _ = trainer.tokenizer.decode(label_ids, skip_special_tokens=True,
                                                        decode_with_timestamps=True).split(",")
            ...
            if cut.recording_id not in processed_sessions:
                processed_sessions[cut.recording_id] = []
            processed_sessions[cut.recording_id].extend(
                process_session(session_preds, trainer.tokenizer, "SOT_FAKE_SPEAKER", cut, save_dir=output_dir)
            )
