...
def calc_session_tcp_loc(ref, hyp, collar):
    res = loc.tcp_loc(reference=ref, hypothesis=hyp, collar=collar)
    res_df = pd.DataFrame.from_dict(res, orient='index').reset_index(names='session_id')
    keys = ['loc_error_rate', 'errors', 'total_errors', 'length', 'leaks', 'omissions', 'confusions', 'others']
    return (res_df[['session_id'] + keys]
            .rename(columns={k: 'tcploc_' + k for k in keys})
            .rename(columns={'tcploc_loc_error_rate': 'tcploc_wer'}))
...
def calc_wer(out_dir: Path,
    ...
    if "tcploc_wer" in metrics_list:
        tcp_loc_res = calc_session_tcp_loc(ref_seglst, tcp_hyp_seglst, collar=collar)
        wers_to_concat.append(tcp_loc_res.drop(columns='session_id'))
