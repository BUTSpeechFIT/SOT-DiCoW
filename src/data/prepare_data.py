...
from data.mappings import ns_mapping_inverted2
...
def main():
    ...
    parser.add_argument("--mapping", default=None, help="Mapping ID of SegID -> RecID")
    args = parser.parse_args()

    if args.mapping is not None:
        if args.mapping == "format1":
            from mappings import ns_mapping_inverted
            seg_rec_map = ns_mapping_inverted
        elif args.mapping == "format2":
            from mappings import ns_mapping_inverted2
            seg_rec_map = ns_mapping_inverted2
        elif args.mapping == "format3":
            from mappings import ns_mapping_inverted3
            seg_rec_map = ns_mapping_inverted3
        elif args.mapping == "format4":
            from mappings import ns_mapping_inverted4
            seg_rec_map = ns_mapping_inverted4
        else:
            raise ValueError("Unknown value: {}".format(args.mapping))
...
def prepare_nsf(
    ...
    num_jobs=1,
    seg_rec_map: Optional[dict] = None,
):
        ...
        if train_diar_dir is not None:
            for use_soft_labels in soft_diar_options:
                _, manifests['train_diar'] = prepare_diar_cutset(
                    ...
                    use_soft_labels=use_soft_labels, seg_rec_map=seg_rec_map,
...
def prepare_diar_cutset(diar_root_dir: str, recordings: RecordingSet, use_soft_labels=False, soft_type="soft_activations", seg_rec_map=None, soft_diar_kwargs={}):
    ...
    if use_soft_labels:
        ...
        sups = []
        for f in diar_dir.rglob(f"*_{soft_type}.npy"):
            sid = Path(f).name.replace(f"_{soft_type}.npy", '')
            rid = seg_rec_map.get(sid, sid) if seg_rec_map is not None else sid
            try:
                rec = recordings[rid]
            except:
    ...
    if seg_rec_map is not None:
        def remap_ids(sup: SupervisionSegment):
            orig_session_id = seg_rec_map.get(sup.recording_id, sup.recording_id)
